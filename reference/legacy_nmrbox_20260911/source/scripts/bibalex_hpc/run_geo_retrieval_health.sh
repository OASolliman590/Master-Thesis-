...
REQUEST_MEM="${REQUEST_MEM:-8G}"
REQUEST_TIME="${REQUEST_TIME:-01:00:00}"
...
cat > /tmp/ici_bibalex_geo_health.sh <<EOF_REMOTE
#!/usr/bin/env bash
...
echo "track	${TRACK}"
'${BIBALEX_CONDA_BIN}' run -n '${BIBALEX_CONDA_ENV}' bash scripts/run_timer_t7_download_full.sh '${DATA_ROOT}' --health-only --track=${TRACK}
EOF_REMOTE

scp /tmp/ici_bibalex_geo_health.sh "${BIBALEX_SSH_HOST}:${REMOTE_SCRIPT}"
