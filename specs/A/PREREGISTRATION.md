...
| A04 | Can clinical comparisons use the same patients? GSE91061 has51 baseline title codes,49 labelled; GSE176307 has89 labelled records | Verified metadata, final eligibility open | candidate_label_coverage.json; `visit`, `response`, `io.response`, downloaded source hashes | Timing/overlap/scale checks remain; counts are ceilings |
...
| A08 | Are49+89 adequate for the proposed precision? Unknown | Unresolved factual/design dependency | Current category counts; no expression pilot or precision calculation | Admission and precision analysis required before readiness |
...
All cited sources were checked in the source audit on5 September2026; source publication dates and retrieval fields are retained there. Claims requiring matrix inspection remain open even when metadata is available. Proposed numerical settings are choices, not facts supplied by a paper.
