...
The current configuration must declare `primary_choice=A-P2` under the 11 September user amendment and separate enabled modules: discovery primary, fixed-score sensitivity secondary, molecular transport and clinical transport. Legacy unresolved/A-P1 values may be read as historical records but cannot release current confirmatory execution. Source-audit work may proceed without scoring. A required frozen-source failure yields `incomplete_primary`, never an apparently successful reweighted aggregate; optional branch failures remain explicit without being promoted to primary evidence.
