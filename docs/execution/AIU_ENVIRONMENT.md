...
# M1 environment provenance addendum

On 5 September 2026, imports succeeded for NumPy2.4.6, pandas2.3.3, SciPy1.17.1 and scikit-learn1.9.0 under the existing Python3.11.16 executable. `aiu_environment_metadata.json` records 376 installed Conda package records and 179 Python distributions, including public source URLs/builds and available archive hashes from installed metadata. Five records have no admitted public URL. This is a provenance inventory, not a recreated environment or verified package-archive lock. `aiu_omics_py_explicit.txt` is the unmodified micromamba list export; its heading and absence of a complete hash-verified lock prevent claiming it is a tested environment recreation input. Pip freeze alone contains build-machine file URLs and was not adopted as a portable lock. No shared environment was modified.
