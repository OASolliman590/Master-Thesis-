...
| B-G1 registry/coverage audit | Corrected B-KIT v0.2 defines deterministic registry, annotation-manifest, complete/incomplete and synthetic-test contracts. | Released for Grok implementation after the amendment commit; no real annotation or patient data run is authorized. |
...
| 3 | B-G1 gene-set registry/coverage audit (B-R1/B-I2 seam) | Validate proposed primary, planned-secondary and M0-M6 registry metadata; audit only supplied normalized annotations. Scoring is explicitly outside B-G1. | Local synthetic implementation/review may proceed. Exact hashes, deterministic complete/incomplete receipts and missing/duplicate/alias coverage rules must pass. Real annotations and all scores remain gated. |
