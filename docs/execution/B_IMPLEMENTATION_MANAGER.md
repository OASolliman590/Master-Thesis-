...
| B-G1 registry/coverage audit | Grok session `01a08b2a-4215-75f3-920e-97925ae23596` implemented the released ticket. Parent review closed duplicate-member, schema-integrity, alias/non-collapse and v0.2 scope-lock gaps. Final local Python 3.12.10: 15 B-G1 tests pass; the combined Paper B regression passes 19 format, 16 prediction and 15 registry tests. A bounded AIU preflight timed out before authentication and created no remote state. See `docs/validation/B_gene_registry/`. | Parent-accepted local synthetic software checkpoint. AIU/Python 3.11 and real normalized annotation coverage remain pending. No score, model, cohort analysis or scientific release. |
...
| 3 | B-G1 gene-set registry/coverage audit (B-R1/B-I2 seam) | Validate proposed primary, planned-secondary and M0-M6 registry metadata; audit only supplied normalized annotations. Scoring is explicitly outside B-G1. | Local synthetic checkpoint accepted. AIU/Python 3.11 and real normalized annotations remain pending; all scores remain gated. |
