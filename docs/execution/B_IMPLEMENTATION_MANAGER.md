...
| B-G2 annotation-normalization spec | Proposed plan, schema and implementation-ready ticket drafted for the three B-G1 required identifier sources. No converter, tests, source download or pinned export. | Specification-only. Real normalized annotation coverage remains pending. |
...
| 3b | B-G2 annotation normalization (feeds B-G1 real coverage) | Patient-free identifier preparation for `tcga_gencode_v36`, `gse107299_processed` and `historical_v18_maps` into the B-G1 table/manifest. Converter not implemented. | Spec kit drafted. Implementation requires a later freeze receipt per source before `source_status=pinned`. Does not freeze U or construct features. |
