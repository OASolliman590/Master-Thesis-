...
| B-G1 registry/coverage audit | Grok session `01a08b2a-4215-75f3-920e-97925ae23596` implemented the released ticket. Parent review closed duplicate-member, schema-integrity, alias/non-collapse and v0.2 scope-lock gaps. Final local Python 3.12.10: 15 B-G1 tests pass; the combined Paper B regression passes 19 format, 16 prediction and 15 registry tests. After one pre-authentication timeout, the exact hash-matched files passed all 15 tests on AIU Python 3.11.16 in 2.430 s. See `docs/validation/B_gene_registry/`. | Parent-accepted local and AIU synthetic software checkpoint. Real normalized annotation coverage remains pending. No score, model, cohort analysis or scientific release. |
| B-G2 annotation normalization | Python 3.11 standard-library-targeted converter and 16 patient-free acceptance tests implemented. Local Python 3.12.10 gates pass; the final combined synthetic Paper B regression passes 66 tests in 33.175 seconds. AIU reached Python 3.11.16, but private-code transfer was blocked before egress, so no remote B-G2 test ran. See `docs/validation/B_annotation_normalize/`. | Parent-accepted local synthetic software checkpoint. AIU portability and every real normalized annotation freeze remain pending. |
...
| 3 | B-G1 gene-set registry/coverage audit (B-R1/B-I2 seam) | Validate proposed primary, planned-secondary and M0-M6 registry metadata; audit only supplied normalized annotations. Scoring is explicitly outside B-G1. | Local and AIU Python 3.11 synthetic software checkpoints accepted. Real normalized annotations remain pending; all scores remain gated. |
| 3b | B-G2 annotation normalization (feeds B-G1 real coverage) | Patient-free identifier preparation for `tcga_gencode_v36`, `gse107299_processed` and `historical_v18_maps` into the B-G1 table/manifest. Converter implemented and locally accepted with synthetic software fixtures. | AIU portability remains pending. Every real source still requires a later parent freeze receipt before `source_status=pinned`; this does not freeze U or construct features. |
